...
var prevHash = hash.dsha256(hexTransaction)
...
// Input
tx.inputsCount = utils.numToVarInt(1)
tx.ins = []
tx.ins[0] = {}
...
tx.ins[0].scriptSigSize = utils.numToVarInt(tx.ins[0].scriptSig.length)
tx.ins[0].sequence = new Buffer('ffffffff', 'hex')

// Output
tx.outputsCount = utils.numToVarInt(1)
tx.outs = []
tx.outs[0] = {}
tx.outs[0].value = new Buffer(8)
tx.outs[0].value.clear()
utils.writeUInt64LE(tx.outs[0].value, amount, 0)
...
tx.outs[0].scriptPubKeySize = utils.numToVarInt(tx.outs[0].scriptPubKey.hex.length)
...
var hashScriptless = hash.dsha256(transaction)
...
// verification that hash160 of public key matches scriptPubKey of output of redeemed tx
//console.log(hash.hash160(public).toString('hex'));
